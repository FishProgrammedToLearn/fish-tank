	...
	<script>
		$(function(){
			...
			$(document).on('click',"[name='attention']",function(){
				...
				$.ajax({
				    type: "POST",
				    url: "/index.php/Home/LeftMyAttention/Attention",
			...
			$(document).on('click',"[name='cancelAttention']",function(){
				...
				$.ajax({
				    type: "POST",
				    url: "/index.php/Home/LeftMyAttention/cancelAttention",
				    data: "nickname="+nickname,
					async:false,
					success:function(data){
						m = data;
        ...
        <div class="row">
			<?php if($info): if(is_array($info)): foreach($info as $key=>$k): ?><div class="col-sm-4">
						<div class="contact-box">
								...
								<a href="/index.php/Home/LeftMyAttention/viewDetails/nickname/<?php echo ($k['nickname']); ?>" class='btn btn-primary btn-sm' style='color:white'>查看信息</a>&nbsp;&nbsp;
								<a href="/index.php/Home/LeftMyAttention/viewDynamic/nickname/<?php echo ($k['nickname']); ?>" class='btn btn-warning btn-sm' style='color:white'>查看动态</a>
			...
			<?php else: ?>
				<div class="middle-box text-center animated fadeInDown">
					<h1 style='font-size:40px;'>您当前并未关注他人~~~</h1>
					<br/>
					<div class="error-desc">
						<a href="<?php echo U('Home/Dynamic/hot');?>">去热门动态动寻找你喜欢的人吧~~~</a>
						<br/>
						<br/>
